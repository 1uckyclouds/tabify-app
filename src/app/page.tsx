...
export default function Home() {
  ...
  const handleBatchRestore = async () => {
    try {
      ...
      addToast(
        `成功恢复 ${selectedTabsData.length} 个标签页`,
        'success',
        '批量恢复成功'
      );
    } catch (error) {
  ...
  const handleCreateGroupSubmit = async (groupData: {
    ...
    try {
      ...
      if (selectedTabs.length > 0) {
        ...
        setSelectedTabs([]);
        addToast(`已创建分组"${groupData.name}"并添加了 ${selectedTabs.length} 个标签页`, 'success');
      } else {
  ...
  const addToast = (message: string, type: 'success' | 'error' | 'warning' | 'info', title?: string, onUndo?: () => void) => {
    ...
    const toast: Toast = {
      ...
      title: title || '',
      message,
      duration: type === 'error' ? 0 : 5000, // 错误提示不自动消失
      showUndo: !!onUndo,
      onUndo,
      createdTime: now,
    };
